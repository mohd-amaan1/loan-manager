        ...
        <table className="w-full table-auto">
          ...
          <tbody>
            {loans.slice((currentPage - 1) * rowsPerPage, currentPage * rowsPerPage).map((loan) => (
              <tr key={loan.id} className="border-b hover:bg-white-50 transition">
                {/* User Details */}
                ...
                <td className="py-4 px-4 flex items-center space-x-2">
                  <span className={`px-3 py-1 rounded-full text-xs font-bold ${loan.status === 'VERIFIED' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
                    {loan.status}
                  </span>
